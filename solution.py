...
==============================================================
Student ID: 2A202600501  (<-- Thay XXXX bang ma so cua ban)
Name: Trịnh Ngọc Tú
...
def validate(data):
    ...
    if error_count > 0:
        print(f"Records dropped: {error_count} (invalid price or empty category)")
...
def transform(data):
    ...
    print(f"Transformation complete. {len(df)} records updated.")
    return df
...
def load(df, output_path):
    ...
    try:
        ...
        print(f"Error saving CSV to '{output_path}': {exc}")
